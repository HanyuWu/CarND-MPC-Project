...
double ref_v = 70;      // The car is designed to run at 70 m/s
size_t x_start = 0;       // Car starts from 0 m/s          
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      if (t > 2) {    // use previous actuations to mitigate latency (100ms approximately)
        a = vars[a_start + t - 3];
        delta = vars[delta_start + t - 3];
      }
